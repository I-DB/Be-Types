...
import * as auth from '../middlewares/jwt/jwt-util'
...
class UserController implements Controller {
	...
	userJoin = async (
		...
		try {
			...
			if (chkExist)
				return next(
					new HttpException(400, '이메일 또는 닉네임이 이미 존재합니다')
				)
	...
	login = async (
		...
		try {
			...
			if (!chkUser)
				return next(
					new HttpException(401, '아이디 또는 비밀번호를 다시 확인해주세요')
				)
